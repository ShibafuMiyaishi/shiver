...
"""
STAGE 1: ベース画像生成
Gemini 2.5 Flash Imageでアニメキャラのベース画像を生成する。
Geminiが利用不可の場合はSD WebUI（Illustrious XL v2.0）にフォールバック。
"""
import asyncio
import base64
import random
...
class BaseImageGenerator:
    """
    ベース画像生成レイヤー。Gemini優先、SD WebUIフォールバック。
    生成した1枚絵がパーツ個別生成（STAGE 2）の「リファレンス画像」になる。
    """
    ...
    async def _generate_sd(self, prompt: str, num_images: int) -> list[str]:
        ...
        async with httpx.AsyncClient(timeout=120.0) as client:
            ...
            res.raise_for_status()
            return res.json()["images"]  # base64リスト
...
# ============================================================
async def call_with_retry(coro_fn, max_retries: int = 5, base_delay: float = 2.0):
    for attempt in range(max_retries):
        try:
            ...
            print(f"[Retry] ⚠️ レート制限 429。{wait_sec:.1f}秒後にリトライ ({attempt + 1}/{max_retries})")
...
class PartsGenerator:
    ...
    def __init__(self, gemini_api_key: str):
        ...
        # ⚠️ 並列処理には必ず .aio（async client）を使うこと
        # self.client.models.generate_content() は同期クライアントであり、
        # async def 内で呼ぶとイベントループをブロックして並列が機能しない。
        # Google Gen AI SDK の公式 async client は client.aio を使う。
        self.async_client = self.client.aio
        self.model = CURRENT_MODEL
        print(f"[PartsGenerator] 使用モデル: {self.model}")
        if self.model == EXPERIMENT_MODEL:
            print("[PartsGenerator] 🧪 実験フェーズ中。品質確認後は CURRENT_MODEL を PRODUCTION_MODEL に変更すること。")
    ...
    async def generate_all_parts(
        ...
        同一レイヤー内のパーツは並列実行。レイヤー間は順次実行。

        推定所要時間:
          5レイヤー × 約15秒 ≒ 約75秒（実験: Gemini 2.5 Flash）
          完全並列の場合は約15秒だが矛盾リスクあり。
        """
        semaphore = asyncio.Semaphore(semaphore_size)
        completed: dict[str, str] = {}   # {part_name: rgba_png_b64}

        for layer_idx, layer in enumerate(self.GENERATION_LAYERS):
            print(f"[PartsGenerator] LAYER {layer_idx} 生成開始: {list(layer.keys())}")
            ...
            for part_name, result in zip(layer_tasks.keys(), results):
                if isinstance(result, Exception):
                    print(f"[PartsGenerator] ⚠️ {part_name} 生成失敗: {result}")
    ...
    async def _call_gemini(self, contents) -> str:
        # .aio.models.generate_content() が正しい async client の呼び方
        # client.models.generate_content() は同期版のため使用禁止
...
### 20.8 Gemini APIのレート制限（429エラー）対策【外部レビュー指摘】

`asyncio.Semaphore(4)` による並列制御は正しい設計だが、APIの無料枠やプレビュー版モデルを使用する場合、**1分あたりのリクエスト数制限**に引っかかる可能性がある。Exponential Backoff（指数バックオフ）による自動リトライを実装する。

```python
# backend/services/parts_generator.py に追加
...
async def call_with_retry(
    ...
    """
    429エラー（レート制限）時にExponential Backoffで自動リトライする。
    ...
    for attempt in range(max_retries):
        try:
            ...
            if not is_rate_limit or attempt == max_retries - 1:
                raise  # レート制限以外のエラー、または最終リトライは即例外

            wait_sec = base_delay * (2 ** attempt) + random.uniform(0, 1)
            print(f"[PartsGenerator] ⚠️ レート制限 429。{wait_sec:.1f}秒後にリトライ "
...
# _generate_single_part() 内での使用例:
# result = await call_with_retry(
#     lambda: self._call_gemini_api(base_image_b64, mask_b64, part_name)
# )
```

> **レート制限について:**
> - 実際のRPM（1分あたりリクエスト数）制限はモデル・プランのtierによって異なり、随時変更される。
> - **断定した数値をここに書かない。** Google AI Studioの「Rate Limits」タブで現在値を確認すること。
>   → https://aistudio.google.com/ → 対象モデルのAPI情報
> - Semaphore(4) + 17パーツの構成でレート制限エラー（429）が出た場合は、
>   `semaphore_size` を下げるか、有料API（Pay-as-you-go）への切り替えを検討する。
> - Exponential Backoffリトライ（`call_with_retry()`）は必ず実装すること。
...
| Geminiインペイントが途中で止まる | 429レート制限 | Exponential Backoffリトライを実装する（20.8参照）。または有料APIに切り替え |
